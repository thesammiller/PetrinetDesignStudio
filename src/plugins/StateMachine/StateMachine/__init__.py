...
class StateMachine(PluginBase):
  ...
  '''
  State Machine
    a petri net is a state machine 
    if every transition has exactly one inplace
    and one outplace.  
  '''
  def testStateMachine(self):
    ...
    check_inplaces = [i[1] for i in inplaces]
    check_outplaces = [o[0] for o in outplaces]
    if len(check_inplaces) == len(set(check_inplaces)) and len(check_outplaces) == len(set(check_outplaces)):
      ...
      logger.info("THIS IS NOT A STATE MACHINE.")
